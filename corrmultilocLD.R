corrmulti.LD<-function(df, loci=c(1,2)){
    multiloc.LD<-function(df, out=c(D), loci=c(1,2)){
      if (length(loci) == 2) {
      ...
      } else {out<-"Linkage can only be calculated for between 0 and 5 loci. Please adjust 'loci'. Default includes loci 1 and 2"}
    ...
    if (length(loci) ==  2) {
      ...
      p.A<-sum(offspring_sub[,1])/(dim(offspring_sub)[1])
      p.B<-sum(offspring_sub[,2])/(dim(offspring_sub)[1])

      D.corr<-multiloc.LD(offspring_sub, loci=c(1, 2))

      } else if (length(loci) == 5) {

      p.A<-sum(offspring_sub[,1])/(dim(offspring_sub)[1])
      p.B<-sum(offspring_sub[,2])/(dim(offspring_sub)[1])
      p.C<-sum(offspring_sub[,3])/(dim(offspring_sub)[1])
      p.D<-sum(offspring_sub[,4])/(dim(offspring_sub)[1])
      p.E<-sum(offspring_sub[,5])/(dim(offspring_sub)[1])

      #calculate frequency of full haplotype
      c.abcde<-0
      for (r in 1:dim(offspring_sub)[1]){
        if (sum(offspring_sub[r,]) == 5) {c.abcde<-c.abcde+1}
      }
      p.ABCDE<-c.abcde/dim(offspring_sub)[1]
      ...
      p.ABCD<-c.abcd/dim(offspring_sub)[1]
      ...
      p.ABC<-c.abc/dim(offspring_sub)[1]
      ...
      D.corr<-p.ABC - p.A*D.BC - p.B*D.AC - p.C*D.AB-p.A*p.B*p.C
